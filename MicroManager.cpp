...
void MicroManager::doMicro(std::list<UnitGroup> listUG)
{
	...
	for(std::list<UnitGroup>::iterator it=listUG.begin(); it!=listUG.end(); it++)
	{
		...
		if((*it)(Scourge).size() > 0)
		{
			...
			UnitGroup airenemies = enemiesInSeekRange(eerste->getPosition(), 7.00, 2);
			UnitGroup allenemies = enemiesInSeekRange(eerste->getPosition(), 7.00, 0);
			if(eerste->isUnderStorm() || canAttackAir(allenemies) && airenemies.size() == 0)
			{
				it->attackMove(hc->getNearestHatchery(eerste->getPosition())->getPosition());
			}
			else
			{
				if(airenemies.size() > 0)
				{
					if(canAttackAir(allenemies) || airenemies.size() > 2*(*it).size())
					{
						moveAway(*it);
					}
					else
		...
		else if((*it)(Mutalisk).size() > 0)
		{
			...
			else if(eerste->getGroundWeaponCooldown() != 0)
			{
				...
				else if(eerste->getDistance(this->tm->findTaskWithUnit(eerste).position) < 9.00)
				{
					moveToNearestBase(*it);
				}
			}
		...
		else if((*it)(Zergling).size() > 0)
		{
			...
			{
				UnitGroup enemies = enemiesInSeekRange(eerste->getPosition(), 10.00, 1);
				...
				else
				{
					...
					if(swarm != NULL && swarm->getPosition().getDistance(eerste->getPosition()) < 9.00)
					{
						...
						{
							(*it).attackMove(swarm->getPosition());
						}
						else
						{
							(*it).attackUnit(nearestUnit(eerste->getPosition(), enemiesInRange(eerste->getPosition(), 20.00, 1)));
						}
					}
					else // <-
					{
						if((*it).size() > 7)
						{
							...
							{
								BWAPI::Unit* nearest = nearestUnit(eerste->getPosition(), enemiesInRange(eerste->getPosition(), 20.00, 1).not(isBuilding).not(isWorker));
								if(true)
								{
									...
									{
										BWAPI::Unit* enemy = nearestUnit(eerste->getPosition(), enemiesInRange(eerste->getPosition(), 20.00, 1));
										UnitGroup enemyUG = enemiesInRange(enemy->getPosition(), 8.00, 1);
										...
										{
											it->attackMove(enemy->getPosition());
										}
									}
								}
								else
			...
			for(std::set<BWAPI::Unit*>::iterator unitit=it->begin(); unitit!=it->end(); unitit++)
			{
				...
				else if((*unitit)->getType() == BWAPI::UnitTypes::Zerg_Lurker)
				{
					if(!(*unitit)->isBurrowed())
					{
						...
						{
							if((*unitit)->getPosition().getDistance(currentTask.position) > 6.00 && enemiesInSeekRange((*unitit)->getPosition(), 13.00, 0).size() == 0)
							{
								(*unitit)->attackMove(currentTask.position);
							}
							else
					...
					else
					{
						...
						{
							UnitGroup enemies = enemiesInSeekRange((*unitit)->getPosition(), 6.00, 0);
							if(enemiesInSeekRange((*unitit)->getPosition(), 13.00, 0).size() > 0)
							{
								if(!this->eiudm->unitIsSeen(*unitit))
								{
									if(enemies.size() > 3)
									{
										if(enemies(Marine).size() > 0 || enemies(isWorker).size() > 0 || enemies(Zealot).size() > 0 || enemies(Medic).size() > 0)
										{
					...
					else
					{
						...
						if(t.type == 1 || t.type == 4)
						{
							if((*unitit)->getPosition().getDistance(nearestEnemyThatCanAttackAir(*unitit)->getPosition()) < 8.00 && t.type == 1)
							{
								...
								else
								{
									...
									{
										(*unitit)->attackMove(neareststealth->getPosition());
									}
									else
									{
										...
										if(dropships.size() > 0)
										{
											(*unitit)->attackMove(nearestUnit((*unitit)->getPosition(), dropships)->getPosition());
										}
										else
										{
											(*unitit)->attackMove(t.position);
						...
						else
						{
							...
							if(hydratask != NULL)
							{
								...
								(*unitit)->attackMove(volghydra->getPosition());
							}
							else
							{
								...
								if(overlordsnearby.size() > 0)
								{
									...
									{
										(*unitit)->attackMove(splitup(*unitit));
									}
								}
								else
								{
									...
									if(buildings.size() > 0)
									{
										...
										(*unitit)->attackMove(BWAPI::Position(newx, newy));
									}
									else
									{
										BWAPI::Unit* nearestbuilding = nearestUnit((*unitit)->getPosition(), allSelfUnits(isBuilding));
										(*unitit)->attackMove(nearestbuilding->getPosition());
									}
								}
							}
						}
							...
							if(canAttackGround(enemiesInRange((*unitit)->getPosition(), 5.00, 0)) || this->eiudm->lostHealthThisFrame(*unitit))
							{
								...
								else
								{
									...
									else
									{
										...
										{
											(*unitit)->attackMove(nearestUnit((*unitit)->getPosition(), militaryInRange)->getPosition());
										}
									}
								}
							}
				...
				else if((*unitit)->getType() == BWAPI::UnitTypes::Zerg_Drone)
				{
					if((*unitit)->isUnderStorm())
					{
						(*unitit)->attackMove(moveAway(*unitit, 20.00));
					}
					else
					{
...
{
	unit->attackMove(hc->getNearestHatchery(unit->getPosition())->getPosition());
}

void MicroManager::moveToNearestBase(std::set<BWAPI::Unit*> units)
{
	UnitGroup::getUnitGroup(units).attackMove(hc->getNearestHatchery((*units.begin())->getPosition())->getPosition());
}
